...
/*   Updated: 2020/01/30 16:44:23 by rabduras         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void static	put_left_column(t_fdf *fdf)
{
	...
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, WIN_WIDTH / 4,
	WIN_HEIGHT / 3 + 34 * (-2), 0xFFFFFF, "R  T  F - Right, Top, Front projection");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, WIN_WIDTH / 4,
	WIN_HEIGHT / 3 + 34 * (-1), 0xFFFFFF, "P       - Perspective projection");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, WIN_WIDTH / 4,
	WIN_HEIGHT / 3 + 34 * 0, 0xFFFFFF, "0       - reset zoom & rotation");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, WIN_WIDTH / 4,
...
void static	put_right_column(t_fdf *fdf)
{
	...
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, WIN_WIDTH / 2 + 160,
	WIN_HEIGHT / 3 + 34 * (-1), 0xFFFFFF, "Scroll for zoom in and zoom out");
}
